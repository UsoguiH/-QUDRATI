...
**للتشغيل محلياً مع معاينة الجوال:**

```bash
node tools/serve.js
```

ثم افتح `http://localhost:8080/mobile.html` — يعرض التطبيق داخل إطار بمقاس جهاز حقيقي (٦ أجهزة)، ويحدّث نفسه تلقائياً عند حفظ أي تعديل. الأمر نفسه يطبع عنوان الشبكة المحلية لفتحه على هاتفك الحقيقي.
